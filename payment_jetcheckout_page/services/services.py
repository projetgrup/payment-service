...
class PaymentAPIService(Component):
    ...
    _description = """Common Payment Page API Services"""
    ...
    def payment_prepare(self, params):
        ...
        try:
            ...
            hash = self._check_hash(api, params.hash, params.amount, params.reference)
            if not hash:
                return PrepareResponse(**RESPONSE['no_hash_match'])

            self._create_transaction(api, hash, params)
            return PrepareResponse(hash=urllib.parse.quote_plus(params.hash),**RESPONSE['success'])
    ...
    def _check_auth(self, company, apikey, secretkey=False):
        ...
        return self.env["payment.acquirer.jetcheckout.page.api"].sudo().search(domain, limit=1)

    def _check_hash(self, key, hash, amount, reference):
        hashed = base64.b64encode(hashlib.sha256(''.join([key.api_key, key.secret_key, str(amount), str(reference)]).encode('utf-8')).digest()).decode('utf-8')
        if hashed != hash:
            return False
        return hash
    ...
    def _create_transaction(self, api, hash, params):
        ...
        tx = self.env['payment.transaction'].sudo().create({
            ...
            'jetcheckout_page_product_name': params.product['name'],
            'jetcheckout_page_hash': hash,
            'jetcheckout_page_tx': params.reference,
            'jetcheckout_page_return_url': params.return_url,
        })
        tx.write({
    ...
    def _get_transaction(self, hash):
        return self.env['payment.transaction'].sudo().search([('jetcheckout_page_hash','=',hash)], limit=1)
