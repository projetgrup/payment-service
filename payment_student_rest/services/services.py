...
class StudentAPIService(Component):
    ...
    def list_student(self, params):
        ...
        domain = [("company_id","=", company),("is_sps","=", True),("parent_id","!=", False),("is_company","=", False)]
        if params.name:
            domain.append(("name", "like", params.name))
        if params.vat:
    ...
    def _auth(self, _company, _apikey, _secretkey=False):
        ...
        return self.env["res.student.api"].sudo().search(domain, limit=1)
