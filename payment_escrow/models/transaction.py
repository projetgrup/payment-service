...
class PaymentTransaction(models.Model):
    ...
    conveyance_sent_date = fields.Datetime('Conveyance Sent Date', readonly=True, copy=False)
    conveyance_status = fields.Selection([
        ('pending', 'Pending'),
        ('uploaded', 'Uploaded'),
        ('sent', 'Sent'),
        ('processed', 'Processed')
    ], string='Conveyance Status', default='pending', readonly=True, copy=False)
    ...
    def _paylox_send_done_mail(self):
        ...
        try:
            with self.env.cr.savepoint():
                ...
                template = self.env.ref('payment_escrow.mail_transaction_successful')
    ...
    def _paylox_send_done_sms(self):
        ...
        try:
            with self.env.cr.savepoint():
                ...
                followers = self.env['mail.followers'].search([('res_model', '=', 'res.partner'), ('res_id', '=', commercial_partner.id)])
                partners = followers.mapped('partner_id') | partner
                template = self.env.ref('payment_escrow.sms_transaction_successful')
    ...
    def paylox_send_daily_email(self):
        users = self.env.ref('payment_escrow.group_escrow_manager').mapped('users')
        template = self.env.ref('payment_escrow.mail_transaction_daily')
        now = datetime.now()
        ...
        for user in users:
            ...
            for company in user.company_ids.filtered(lambda x: x.system == 'escrow'):
