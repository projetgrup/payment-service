        ...
        if self.company_id.syncops_sync_item_split:
            for item in self.paylox_transaction_item_ids:
                ...
                result, message = self.env['syncops.connector'].sudo()._execute('payment_post_partner_payment', reference=str(self.id), params={
                    'id': item.item_id.id,
                    'ref': child_ref or ref or '', #TODO
                    'vat': child_vat or vat or '', #TODO
                    'name': name or '',
                    ...
                    'amount_customer_rate': 0 if self.source_transaction_id else abs(self.jetcheckout_customer_rate or 0),
            ...
            params = {
                ...
                'installments': self.jetcheckout_installment_description or '',
                'installment_description': self.jetcheckout_installment_description_long or '',
                'amount_net': float_round(abs(self.amount * (100 - self.jetcheckout_commission_rate) / 100) if self.source_transaction_id else abs(self.jetcheckout_payment_net or 0), 2),
                'amount_commission_cost': 0 if self.source_transaction_id else abs(self.jetcheckout_commission_amount or 0),
                'amount_customer_cost': 0 if self.source_transaction_id else abs(self.jetcheckout_customer_amount or 0),
                'amount_commission_rate': 0 if self.source_transaction_id else abs(self.jetcheckout_commission_rate or 0),
                'amount_customer_rate': 0 if self.source_transaction_id else abs(self.jetcheckout_customer_rate or 0),
            ...
            if self.jetcheckout_payment_type == 'physicalpos':
                params.update({
                   ...
                   'type_physicalpos_transaction_id': self.jetcheckout_payment_type_physicalpos_transaction_id or '',
                   'type_physicalpos_batch_id': self.jetcheckout_payment_type_physicalpos_batch_id or '',
