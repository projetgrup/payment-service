...
class PayloxSystemController(PayloxController):
    ...
    def _check_payment_preview_page(self):
        if not request.env.user.payment_preview_ok or not request.env.company.payment_page_ok:
            raise werkzeug.exceptions.NotFound()
    ...
    def page_system_link_item_add(self, **kwargs):
        ...
        payment_desc = str(kwargs.get('desc') or '')
        payment_desc_minlength = company.payment_page_item_add_desc_minlength
        if payment_desc_minlength and (len(payment_desc) < payment_desc_minlength):
        ...
        if payment_desc_maxlength and len(payment_desc) > payment_desc_maxlength:
        ...
        payment_desc_prefix = company.sudo()._get_payment_page_item_add_desc_prefix(partner)
        ...
        request.env['payment.item'].sudo().create({
            ...
            'description': payment_desc or False,
        })
